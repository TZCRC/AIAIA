"""
Script to draw bbox over original images, in order to check the correct data input

Author: @developmentseed

Run:
    python3 draw_bboxes_over_original_images.py \
        --csv=RR17_train.csv \
        --out_dir=RR17_bbox
...
def draw_bbox(image, df, out_dir, label_text):
    """Draw the bbox on the images

    Args:
        img(str): image path
        df(Data frame):
        out_dir(str): path for the output images
    ...
    if os.path.exists(image):
        ...
        for attri in attributes:
            ...
            draw.rectangle(bbox, outline=class_color(int(attri[2])), width=10)
            if label_text:
                draw.text(xy=bbox[1], text=str(attri[1]),
                          fill='red', align="right")
        im.save(f'{out_dir}/{img_name}_bboxes.png', "PNG")
    else:
        print(f'No found image...{image}')
...
@click.option('--out_dir', help='Path to save RGB images with the bbox drawed')
def main(csv, out_dir, label_text=True):
    ...
    Parallel(n_jobs=-1)(
        delayed(draw_bbox)(image, df, out_dir, label_text)
